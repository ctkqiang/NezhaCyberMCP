...
// 以 GHSA ID（主键）作为冲突判断依据，冲突时更新所有可变字段，确保数据始终为最新状态。
//
// 参数：
//   - ctx      : 请求上下文
...
func (r *GithubAdvisoryRepository) BulkUpsert(ctx context.Context, advisories []model.GithubAdvisory) error {
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		result := tx.
			...
			CreateInBatches(advisories, 100) // 每批 100 条，避免单条 SQL 过长
	...
	if err != nil {
		...
		return fmt.Errorf("bulk upsert %d advisories: %w", len(advisories), err)
	}
